...
export default class QuizSaver {
	...
	constructor(app: App, settings: QuizSettings, quizSources: TFile[]) {
		...
		const normalizedPath = normalizePath(this.settings.savePath);
		const folder = this.app.vault.getAbstractFileByPath(normalizedPath);
		// A path is valid if it's root, or an existing folder, or we're about to create it
		this.validSavePath = normalizedPath === "" || normalizedPath === "/" || normalizedPath === "." || (folder !== null && folder instanceof TFolder) || (normalizedPath.length > 0);
	}
	...
	private getSaveFilePath(): string {
		let count = 1;
		const normalizedPath = normalizePath(this.settings.savePath);
		const saveFolder = this.app.vault.getAbstractFileByPath(normalizedPath);
		const validSavePath = normalizedPath !== "" && normalizedPath !== "/" && normalizedPath !== "." && saveFolder instanceof TFolder;

		const fileNames = validSavePath ? this.getFileNames(saveFolder as TFolder) : this.getFileNames(this.app.vault.getRoot());
		...
		return normalizedPath !== "" && normalizedPath !== "/" && normalizedPath !== "."
			? normalizePath(`${normalizedPath}/Quiz ${count}.md`)
			: `Quiz ${count}.md`;
	}
