...
import displayClaudeCodeSettings from "./claude-code/claudeCodeSettings";
...
const displayModelSettings = (containerEl: HTMLElement, plugin: QuizGenerator, refreshSettings: () => void): void => {
	...
	} else if (plugin.settings.provider === Provider.CLAUDE_CODE) {
		displayClaudeCodeSettings(containerEl, plugin);
	}
};
