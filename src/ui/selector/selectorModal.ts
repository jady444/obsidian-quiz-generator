...
export default class SelectorModal extends Modal {
	...
	private activateButtons(): Record<SelectorModalButton, HTMLButtonElement> {
		...
		const generateQuizHandler = async (): Promise<void> => {
			...
			try {
				...
				let contents = [...this.selectedNotes.values()];
				if (this.settings.generateFromLinkedNotes) {
					const linkedContents = await this.getLinkedNotesContents();
					contents = [...contents, ...linkedContents];
				}

				if (contents.join("").trim().length === 0) {
					new Notice("No content found in selected or linked notes");
					this.toggleButtons([SelectorModalButton.GENERATE], false);
					return;
				}

				const generatedQuestions = await generator.generateQuiz(contents);
	...
	private async addNote(note: string): Promise<void> {
		...
		if (selectedNote instanceof TFile) {
			...
			await this.recalculatePromptTokens();
		}
	}
	...
	private renderNote(note: TFile): void {
		this.renderNoteOrFolder(note, this.settings.showNotePath ? note.path : note.basename);
	...
	private renderFolder(folder: TFolder): void {
		...
		this.renderNoteOrFolder(folder, folderName);
		this.toggleButtons([SelectorModalButton.CLEAR, SelectorModalButton.GENERATE], false);
	}

	private renderNoteOrFolder(item: TFile | TFolder, fileName: string): void {
	...
	private async getLinkedNotesContents(): Promise<string[]> {
		const linkedContents: string[] = [];
		const processedPaths = new Set<string>(this.selectedNotes.keys());
		const limit = this.settings.linkedNotesLimit;

		for (const [path, files] of this.selectedNoteFiles) {
			for (const file of files) {
				// 1. Forward Links
				const metadata = this.app.metadataCache.getFileCache(file);
				if (metadata?.links) {
					for (const link of metadata.links) {
						if (linkedContents.length >= limit) break;
						...
						if (linkedFile instanceof TFile && !processedPaths.has(linkedFile.path)) {
							...
							linkedContents.push(cleanUpNoteContents(content, getFrontMatterInfo(content).exists));
							processedPaths.add(linkedFile.path);
						}
					}
				}

				// 2. Backlinks (Notes that link TO this file)
				if (linkedContents.length < limit) {
					const allResolvedLinks = this.app.metadataCache.resolvedLinks;
					for (const [sourcePath, links] of Object.entries(allResolvedLinks)) {
						if (linkedContents.length >= limit) break;
						if (links[file.path] && !processedPaths.has(sourcePath)) {
	...
	private async recalculatePromptTokens(): Promise<void> {
		let total = 0;
		for (const content of this.selectedNotes.values()) {
			total += countNoteTokens(content);
		}
		if (this.settings.generateFromLinkedNotes) {
			const linked = await this.getLinkedNotesContents();
			for (const content of linked) {
				total += countNoteTokens(content);
			}
		}
		this.updatePromptTokens(total);
	}
	...
	private validGenerationSettings(): boolean {
		return (this.settings.generateTrueFalse || this.settings.generateMultipleChoice ||
			...
			(this.promptTokens > 0 || this.settings.generateFromLinkedNotes);
	}
}
