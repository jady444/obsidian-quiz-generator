...
- **Linked Notes Support:** Automatically include content from outgoing links and backlinks to generate more comprehensive quizzes (especially useful for "Tag" notes or Maps of Content).
- **Automatic Folder Creation:** Quizzes are saved exactly where you want them, with folders created automatically if they don't exist.
...
- [Claude Code CLI](https://github.com/anthropics/claude-code): Use the powerful Claude Code CLI for high-quality, non-interactive quiz generation.
  - **Note for Mac users**: You must provide the absolute path to the `claude` executable (e.g., `/usr/local/bin/claude`). Run `which claude` in your terminal to find it.
  - **Note for Windows users**: You must provide the absolute path to the `claude.cmd` file (e.g., `C:\Users\YourName\AppData\Roaming\npm\claude.cmd`).
...
If you want to use this plugin in a different vault without downloading it again:
1. Navigate to your current vault's folder: `.obsidian/plugins/quiz-generator/`.
2. Copy the following three files: `main.js`, `manifest.json`, and `styles.css`.
3. Go to your **new vault's** folder, navigate to `.obsidian/plugins/`, and create a new folder named `quiz-generator`.
4. Paste the three files into that new folder.
5. Restart Obsidian or go to **Settings** → **Community plugins** and select the **Reload** icon.
6. Toggle the switch to **Enable** the plugin.

#### Building from Source

If you want to generate the plugin files (`main.js`, `manifest.json`, and `styles.css`) yourself from the source code:
